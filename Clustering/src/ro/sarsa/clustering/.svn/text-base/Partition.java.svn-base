...
public class Partition<T> {
	...
	/**
	 * Create a partition where each cluster has exactly one element
	 * 
	 * @param objects
	 */
	public Partition(List<T> objects) {
		int nrClusters = objects.size();
		clusters = new ArrayList<Cluster<T>>(nrClusters);
		for (int i = 0; i < nrClusters; i++) {
			addSingletonCluster(objects.get(i));
		}
	}
	...
	public void addToCluster(int clusterNr, T ob) {
		clusters.get(clusterNr).add(ob);
	}
}
