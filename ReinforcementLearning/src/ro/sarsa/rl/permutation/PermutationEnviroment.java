...
public abstract class PermutationEnviroment extends Enviroment {
	...
	public List<Action> getPosibleActions(State state, History hist) {
		...
		// List<Action> rez = new
		// ArrayList<Action>(PermutationAction.getAll(permDim));
		// for (int i = 0; i < lg; i++) {
		// rez.remove(PermutationAction.get(perm[i], permDim));
		// }
		// return rez;
		return getNotIn(perm);
	}

	private List<Action> getNotIn(short[] perm) {
		try {
			List<Action> rez = new ArrayList<Action>(permDim - perm.length);
			for (int sIndex = 0; sIndex < permDim; sIndex++) {
				boolean found = false;
				for (short index : perm) {
					if (index == sIndex) {
						found = true;
						break;
					}
				...
				if (!found) {
					rez.add(hackPossibleActions.get(sIndex));
				}
			}
			return rez;
		} catch (Exception ex) {
