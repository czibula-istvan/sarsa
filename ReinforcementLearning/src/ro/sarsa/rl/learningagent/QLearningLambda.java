...
public class QLearningLambda extends ReinforcementLearning 
{
	...
	public QLearningLambda(Enviroment env, int maxSteps, double learningRate,
	...
	public QLearningLambda(Enviroment env, int maxSteps, 
						...
						History hist) 
	{
		...
		{
			List<Action> posActs = env.getPosibleActions(cState, hist);
			for(int i = 0; i < posActs.size(); i++ )
			{
				Action act = posActs.get(i);
				if( act.equals(currentAction))
					continue;
				elTrace.set(cState, posActs.get(i), 0);
			}
		}
		*/
