...
public class QLearningLambda extends ReinforcementLearning {
	...
	public QLearningLambda(Enviroment env, int maxSteps, double learningRate, double discountFactor) {
	...
	public History epoch() {
		...
		do {
			...
			// Action maxAction = this.q.getMax(newS,
			// env.getPosibleActions(newS, null));
			Action maxAction = this.q.getMax(newS, env.getPosibleActions(newS, this.hist));

			this.updateQValues(currentAction, cState, reward, newS, newAction, maxAction, this.hist);
	...
	private void updateQValues(Action currentAction, State cState, double reward, State newState, Action newAction,
		...
		// double maxQVal = q.getMaxValue(newState,
		// env.getPosibleActions(newState, null));
		double maxQVal = q.getMaxValue(newState, env.getPosibleActions(newState, this.hist));
		double tdError = reward + gama * maxQVal - q.get(cState, currentAction);

		elTrace.update(cState, currentAction, 1);

		/*
		 * if( this.elTrace.getType().equals(EligibilityTraceType.Replacing) ) {
		 * List<Action> posActs = env.getPosibleActions(cState, hist); for(int i
		 * = 0; i < posActs.size(); i++ ) { Action act = posActs.get(i); if(
		 * act.equals(currentAction)) continue; elTrace.set(cState,
		 * posActs.get(i), 0); } }
		 */
